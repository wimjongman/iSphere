...
public class FNDSTR_search {
	...
	public int run(
		...
		try {
			...
			ProgramCallDocument pcml = 
				new ProgramCallDocument(
						_as400, 
						"de.taskforce.isphere.sourcefilesearch.FNDSTR_search", 
						this.getClass().getClassLoader());

			pcml.setIntValue("FNDSTR_search.handle", _handle);
			pcml.setValue("FNDSTR_search.string", _string);
			pcml.setIntValue("FNDSTR_search.fromColumn", _fromColumn);
			pcml.setIntValue("FNDSTR_search.toColumn", _toColumn);
			pcml.setValue("FNDSTR_search.case", _case);
		...
		//	e.printStackTrace();
		//	System.out.println("*** Call to FNDSTR_search failed. ***");
		//	return null;
