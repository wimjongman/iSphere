...
import de.taskforce.isphere.messagefileeditor.MessageDescription;
import de.taskforce.isphere.messagefileeditor.MessageDescriptionDetailDialog;
import de.taskforce.isphere.messagefileeditor.MessageFileEditor;
import de.taskforce.isphere.messagefileeditor.QMHRTVM;
			    ...
			    if(tableViewerMessageFiles.getSelection() instanceof IStructuredSelection) {
					...
					if (editor != null) {
						...
						MessageFileEditor.openEditor(
								_searchResult.getAS400(), 
								_searchResult.getHost(), 
								_searchResult.getLibrary(), 
								_searchResult.getMessageFile(),
								"*EDIT");
			...
			public void doubleClick(DoubleClickEvent event) {
				...
				if (selectedItemsMessageFiles != null && selectedItemsMessageFiles.length == 1) {
				    ...
				    if (tableViewerMessageIds.getSelection() instanceof IStructuredSelection) {
						...
						MessageDescription[] _messageDescription = qmhrtvm.run(
								_searchResult.getAS400(), 
								_searchResult.getHost(), 
								_searchResult.getLibrary(), 
								_searchResult.getMessageFile(), 
								_searchResultMessageId.getMessageId());
