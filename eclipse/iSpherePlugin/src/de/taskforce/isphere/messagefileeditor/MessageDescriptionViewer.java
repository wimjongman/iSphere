...
package de.taskforce.isphere.messagefileeditor;
...
import de.taskforce.isphere.internal.DialogActionTypes;
import de.taskforce.isphere.internal.Size;
...
public class MessageDescriptionViewer {
	...
	private class ContentProviderTableViewer implements IStructuredContentProvider {
		public Object[] getElements(Object inputElement) {
			if (messageDescriptions == null) {
				...
				messageDescriptions = qmhrtvm.run(
						as400, 
						connection, 
						library, 
						messageFile, 
		...
		_tableViewer.addDoubleClickListener(new IDoubleClickListener() {
			public void doubleClick(DoubleClickEvent event) {
			    if (_tableViewer.getSelection() instanceof IStructuredSelection) {
					...
					MessageDescriptionDetailDialog _messageDescriptionDetailDialog = new MessageDescriptionDetailDialog(shell, as400, DialogActionTypes.DISPLAY, _messageDescription);
	...
	private void executeMenuItemNew() {
		...
		MessageDescriptionDetailDialog _messageDescriptionDetailDialog = new MessageDescriptionDetailDialog(shell, as400, DialogActionTypes.CREATE, _messageDescription);
		if (_messageDescriptionDetailDialog.open() == Dialog.OK) {
	...
	private void executeMenuItemChange() {
		...
		for (int idx=0; idx < selectedItems.length; idx++) {
			if (selectedItems[idx] instanceof MessageDescription) {
				...
				MessageDescriptionDetailDialog _messageDescriptionDetailDialog = new MessageDescriptionDetailDialog(shell, as400, DialogActionTypes.CHANGE, (MessageDescription)selectedItems[idx]);
				if (_messageDescriptionDetailDialog.open() == Dialog.OK) {
					_tableViewer.update(((MessageDescription)selectedItems[idx]), null);
				}
	...
	private void executeMenuItemCopy() {
		...
		for (int idx=0; idx < selectedItems.length; idx++) {
			if (selectedItems[idx] instanceof MessageDescription) {
				...
				MessageDescriptionDetailDialog _messageDescriptionDetailDialog = new MessageDescriptionDetailDialog(shell, as400, DialogActionTypes.COPY, _messageDescription);
				if (_messageDescriptionDetailDialog.open() == Dialog.OK) {
					_tableViewer.add(_messageDescription);
				}
	...
	private void executeMenuItemDelete() {
		...
		for (int idx=0; idx < selectedItems.length; idx++) {
			if (selectedItems[idx] instanceof MessageDescription) {
				...
				MessageDescriptionDetailDialog _messageDescriptionDetailDialog = new MessageDescriptionDetailDialog(shell, as400, DialogActionTypes.DELETE, (MessageDescription)selectedItems[idx]);
				if (_messageDescriptionDetailDialog.open() == Dialog.OK) {
					_tableViewer.remove(((MessageDescription)selectedItems[idx]));
				}
	...
	private void executeMenuItemDisplay() {
		...
		for (int idx=0; idx < selectedItems.length; idx++) {
			if (selectedItems[idx] instanceof MessageDescription) {
				...
				MessageDescriptionDetailDialog _messageDescriptionDetailDialog = new MessageDescriptionDetailDialog(shell, as400, DialogActionTypes.DISPLAY, (MessageDescription)selectedItems[idx]);
				if (_messageDescriptionDetailDialog.open() == Dialog.OK) {
				}
