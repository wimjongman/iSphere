...
public class TransferTN5250JLibraryAction implements IObjectActionDelegate {

	private ArrayList<TN5250JSubSystem> selectedSubSystems;
	
	public TransferTN5250JLibraryAction() {
	...
	public void run(IAction action) {
		for (int idx = 0; idx < selectedSubSystems.size(); idx++) {
			TransferTN5250JLibrary statusDialog = new TransferTN5250JLibrary(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell().getDisplay(), SWT.APPLICATION_MODAL | SWT.SHELL_TRIM);
			if (statusDialog.connect(selectedSubSystems.get(idx).getHostName())) {
				statusDialog.open();
			}
		}
	}
